...
globalThis.Renderer = function () {
	...
	this._renderEntriesSubtypes_block = function ({entry, textStack, meta, options, incDepth, displayName}) {
		...
		if (entry.name != null && Renderer.ENTRIES_WITH_ENUMERATED_TITLES_LOOKUP[entry.type]) this._handleTrackTitles(entry._nameEn ?? entry.name);
	...
	this._renderInset = function (entry, textStack, meta, options) {
		...
		if (entry.name != null) {
			if (Renderer.ENTRIES_WITH_ENUMERATED_TITLES_LOOKUP[entry.type]) this._handleTrackTitles(entry._nameEn ?? entry.name);
	...
	this._renderVariantInner = function (entry, textStack, meta, options) {
		...
		textStack[0] += `<span class="ve-rd__h ve-rd__h--2-inset" data-title-index="${this._headerIndex++}" ${this._getEnumeratedTitleRel(entry._nameEn ?? entry.name)}><h4 class="entry-title-inner">${entry.name}</h4></span>`;
		const len = entry.entries.length;
		for (let i = 0; i < len; ++i) {
			const cacheDepth = meta.depth;
			meta.depth = 2;
	...
	this._renderActions = function (entry, textStack, meta, options) {
		...
		const headerSpan = `<span class="ve-rd__h ve-rd__h--3" data-title-index="${this._headerIndex++}" ${this._getEnumeratedTitleRel(entry._nameEn ?? entry.name)}><span class="entry-title-inner">${entry.name}.</span></span> `;
	...
	this._renderFlowBlock = function (entry, textStack, meta, options) {
		...
		if (entry.name != null) {
			if (Renderer.ENTRIES_WITH_ENUMERATED_TITLES_LOOKUP[entry.type]) this._handleTrackTitles(entry._nameEn ?? entry.name);
			textStack[0] += `<span class="ve-rd__h ve-rd__h--2-flow-block" data-title-index="${this._headerIndex++}" ${this._getEnumeratedTitleRel(entry._nameEn ?? entry.name)}><h4 class="entry-title-inner">${this.render({type: "inline", entries: [entry.name]})}</h4></span>`;
		}
		if (entry.entries) {
			const len = entry.entries.length;
			for (let i = 0; i < len; ++i) {
			...
			<th class="ve-stats__th-name ve-text-left ve-pb-0 ${opts.extraThClasses ? opts.extraThClasses.join(" ") : ""}" colspan="6" ${dataPart}>
				<div class="ve-split-v-end">
					<div class="ve-flex-v-center">
						<h1 class="ve-stats__h-name ve-copyable ve-m-0" onmousedown="event.preventDefault()" onclick="Renderer.utils._pHandleNameClick(this)">${opts.prefix || ""}${globalThis.ZH ? ZH.nameHtml(ent, name) : name}${opts.suffix || ""}</h1>
						${opts.htmlControlRhs || ""}
	...
	static enhanceItem (item, {styleHint = null} = {}) {
		...
		if (itemTypeAbv === Parser.ITM_TYP_ABV__LIGHT_ARMOR || itemTypeAbv === Parser.ITM_TYP_ABV__MEDIUM_ARMOR || itemTypeAbv === Parser.ITM_TYP_ABV__HEAVY_ARMOR) {
			...
			if (itemTypeAbv === Parser.ITM_TYP_ABV__HEAVY_ARMOR && item.strength) {
				Renderer.item._initFullEntries(item);
				item._fullEntries.push({type: "wrapper", wrapped: `若穿戴者的力量值低於 ${item.strength}，其速度降低 10 呎。`, data: {[VeCt.ENTDATA_ITEM_MERGED_ENTRY_TAG]: "type"}});
			}
		}
		...
		if (item._isItemGroup && item.items?.length && !item.itemsHidden) {
			Renderer.item._initFullEntries(item);
			item._fullEntries.push({type: "wrapper", wrapped: "此物品有多種變體，列於下方：", data: {[VeCt.ENTDATA_ITEM_MERGED_ENTRY_TAG]: "magicvariant"}});
			item._fullEntries.push({
				type: "wrapper",
				wrapped: {
					type: "list",
